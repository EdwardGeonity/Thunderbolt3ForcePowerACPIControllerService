...
DefinitionBlock ("", "SSDT", 2, "Nebula", "TbtCtr17", 0x00000000)
{
    ...
    Scope (_GPE)
    {
        ...
        Method (TBIN, 0, NotSerialized)
        {
            ...
            CreateByteField (TCRB, 0x10, BUSA) //Thunderbolt Bridge Bus Number      
            CreateByteField (TCRB, 0x11, TSA1) ////First Time Boot Status (TSA1) (Byte) 
    ...
    Scope (_SB.PCI0)
    {
        Device (THUN)
        {
            ...
            Method (TDPE, 0, NotSerialized)
            {
                ...
                If (LAnd (LNotEqual (TDUS, Zero), LNotEqual (TDUS, 0xFF)))
                {
                    Add (Local2, 0x10000, Local3) //Create address of Thunderbolt HUB Device
